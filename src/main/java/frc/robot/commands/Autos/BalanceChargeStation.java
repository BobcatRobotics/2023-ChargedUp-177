...
public class BalanceChargeStation extends Command {
  ...
  // for in depth info on PID controllers, see
   ...
   * the PID controller calculates the error (difference between current value and
   * setpoint)
   * and uses it to calculate the output
   * 
   * gains are constants that are used to calculate the output and need to be
   * tuned to
  ...
  public BalanceChargeStation(SwerveDrive dt, boolean isContinuous) {
  ...
  public void execute() {
    ...
    calc = pid.calculate(dt.getBaseGyro().getPitch().getDegrees());
    if (!isOffset) {
      dt.drive(
          new Translation2d(
              throttle(calc / sensitivity) * dt.jsonSwerve.chassisSpeedLimits.maxSpeed,
              0),
          0,
          true,
          dt.getGyroYaw(), dt.getPose());
    }

    SmartDashboard.putNumber("charge error", calc);
    SmartDashboard.putBoolean(" charge Setpoint", pid.atSetpoint());
    SmartDashboard.putNumber("charge output", throttle(calc / sensitivity) * dt.jsonSwerve.chassisSpeedLimits.maxSpeed);
  ...
  public void end(boolean interrupted) {
    dt.drive(new Translation2d(0, 0), 0, true,dt.getGyroYaw(),dt.getPose());
    if (!interrupted) {
